...
from ast import NodeVisitor, Load, parse, Name as AstName, Store, dump
...
class NameExtractor(NodeVisitor):
    ...
    def visit_ExceptHandler(self, node):
        ...
        if node.name:
            ...
            name.is_local = True
            name.id = node.name
            name.lineno, name.col_offset = self.idx_name_extractor.get_first_after(node.lineno, 'as')
            name.ctx = Store()
        ...
        if self.onhold:
            ...
            try:
                ...
                while tid in self.SPACES:
                    tid, value, start, end, _  = next(self.tokens)
